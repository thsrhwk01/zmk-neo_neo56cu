...
Community ZMK firmware for the STM32F072-based **Qwertykeys/NEO Studio Neo65 CU wired PCB**. It provides a USB-only ZMK port and a guarded Windows flasher that preserves the factory STM32 system-ROM bootloader.
...
- Pinned ZMK revision and automatic GitHub Actions packaging
...
- The BIN has a valid STM32F072 size, initial stack pointer, and Reset Handler.
...
| Keys | Action |
| --- | --- |
| Fn + `Backspace` | Restart the ZMK application |
| Hold `Esc` while connecting USB | Enter STM32 system-ROM DFU before ZMK starts |
| Fn + `Delete` | Enter STM32 system-ROM DFU from ZMK |
...
Available ROM DFU entry paths are:

1. Hold `Esc` while connecting USB.
2. From ZMK, hold Fn and press `Delete`.
...
The earlier multi-key reset and ROM-DFU combos were removed because ZMK delays member-key output while waiting for a possible combo. Dedicated Fn-layer bindings avoid that latency during normal typing.

The runtime ROM handoff itself was hardware-tested with the earlier combo. Test the newly rebound Fn-layer keys on hardware before relying on them as the only runtime recovery path.

## Build and release

The main workflow is pinned to ZMK `v0.3.0` commit `edf5c0814fd3ea202e43aad2d68fd32e882a518c`. Like the LINK65 repository, it runs the standard ZMK build once, checks only the raw BIN's size, initial MSP, and Reset Handler, and packages the result with bundle checksums.

Branch pushes, pull requests, and manual workflow runs produce both artifacts below. Version tags additionally publish a GitHub Release.
...
`SHA256SUMS.txt` protects the files within each generated bundle. A successful build is not a substitute for testing changed firmware on the physical keyboard.
...
ROM handoff and hardware-validation process. The chronological evidence

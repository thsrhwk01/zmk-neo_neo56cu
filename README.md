...
- Pinned ZMK revision, lightweight BIN validation, and a hardware-approved release hash
...
The earlier multi-key reset and ROM-DFU combos were removed because ZMK delays member-key output while waiting for a possible combo. Dedicated Fn-layer bindings avoid that latency during normal typing. The runtime behaviors were hardware-tested with the earlier combos; the new bindings require a new hardware-validation pass before their changed BIN hash can replace the approved release manifest.

## Build and release

The main workflow is pinned to ZMK `v0.3.0` commit `edf5c0814fd3ea202e43aad2d68fd32e882a518c`. It runs the standard ZMK build once, checks the raw BIN's size, MSP, reset handler, and interrupt vectors, then validates the packaged flasher without accessing a device.
...
The Windows package is fail-closed against [`release/neo65cu-zmk.sha256`](release/neo65cu-zmk.sha256). Any source or keymap change that produces a different BIN intentionally blocks packaging and release until that new image is hardware-tested, read back, and explicitly approved by updating the manifest.
